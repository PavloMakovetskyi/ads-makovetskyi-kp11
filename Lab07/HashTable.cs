...
namespace HashTable
{
    public class HashTable
    {
        ...
        private int _loadness;          
        private int _size;            
        private int _totalSize;         
        const int _siezedTime = 105;   
        private HashTable _gateFlights; 
        ...
        public HashTable(int gateFlightCount = 10, int totalSize = 20)
        {
            ...
            {
                _gateFlights = new HashTable(0, gateFlightCount);
            }
        }
        public void InsertEntry(Key key, Value value)   
        {
            ...
            _gateFlights?.InsertEntry(new Key(entry.Value.Gate), new Value(entry.Key.FlightCode));


            if (3.0 / 4 <= _loadness / _totalSize && _gateFlights != null)
            {
                Rehashing();
            }
        }
        ...
        public void RemoveEntry(Key key)
        {
            ...
            if(_gateFlights == null)
            {
                return;
            }

            var gateKey = new Key(entry.Value.Gate);    
            int gateTableIndex = _gateFlights.GetHash(gateKey);
            var entryGate = _gateFlights._table[gateTableIndex].Where(x => x.Value == entry.Value).FirstOrDefault();
        ...
        public string[] SameGateFlights(string gate)    
        {
            var gateKey = new Key(gate);
            int? gateTableIndex = _gateFlights?.GetHash(gateKey);
            return _gateFlights?._table[gateTableIndex.Value]?.Select(x=>x.Value.FlightCode)?.ToArray();
        } 
        ...
        {
            _loadness = otherHashTable._loadness;
        ...
        private bool CheckIsFreeGate(Entry entry)
        {
            ...
            foreach (var key in keys)
            {
                ...
                int ey = entry.Value.DepartureTime.Year;
                ...
                if (y == ey && m == em && minutesBais <= _siezedTime)
                {
                    return false;
                }
            }
